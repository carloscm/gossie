...
func (q *query) Get(key []byte) (*Row, error) {
	if q.cf == "" {
	...
	err := q.pool.run(func(c *connection) (*cassandra.InvalidRequestException, *cassandra.UnavailableException, *cassandra.TimedOutException, error) {
...
func (q *query) Count(key []byte) (int, error) {
	if q.cf == "" {
...
func (q *query) MultiGet(keys [][]byte) ([]*Row, error) {
	if q.cf == "" {
		return nil, errors.New("No column family specified")
	}
...
func (q *query) MultiCount(keys [][]byte) ([]*RowColumnCount, error) {
	if q.cf == "" {
		return nil, errors.New("No column family specified")
	}
...
func (q *query) RangeGet(rang *Range) ([]*Row, error) {
	if q.cf == "" {
		return nil, errors.New("No column family specified")
	}
...
func (q *query) IndexedGet(rang *IndexedRange) ([]*Row, error) {
	if q.cf == "" {
		return nil, errors.New("No column family specified")
	}
...
func (m *mutation) Run() error {
	return m.pool.run(func(c *connection) (*cassandra.InvalidRequestException, *cassandra.UnavailableException, *cassandra.TimedOutException, error) {
		var ire *cassandra.InvalidRequestException
		var ue *cassandra.UnavailableException
		var te *cassandra.TimedOutException
		var err error
